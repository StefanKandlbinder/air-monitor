...
export function MapNavigation({
  ...
  return (
    <div className="absolute left-4 bottom-4 flex gap-1 z-10">
      <Button variant="secondary" size="icon" onClick={onZoomIn} aria-label={dict.map.zoomIn}>
        <Plus className="h-4 w-4" />
      </Button>
      <Button variant="secondary" size="icon" onClick={onZoomOut} aria-label={dict.map.zoomOut}>
        <Minus className="h-4 w-4" />
      </Button>
      <Button variant="secondary" size="icon" className="ml-2" onClick={onResetNorth} aria-label={dict.map.resetNorth}>
        <CompassIcon bearing={bearing} className="h-4 w-4 transition-transform" />
      </Button>
      {onCenterOnUserLocation ? (
        <Button
          variant="secondary"
